...
class DeepBeliefNet():    
    ...
    def recognize(self,true_img,true_lbl):
        ...
        p_h_out, h_out = self.rbm_stack['hid--pen'].get_h_given_v_dir(h)
        p_h_out_label = np.concatenate((h_out,lbl), axis=1)

        for _ in range(self.n_gibbs_recog):
            curr_h3, h_top = self.rbm_stack['pen+lbl--top'].get_h_given_v(p_h_out_label) 
            p_h_out_label, v_top = self.rbm_stack['pen+lbl--top'].get_v_given_h(curr_h3) 
            

        predicted_lbl = v_top[:, -self.sizes['lbl']:]
    ...
    def generate(self,true_lbl,name):
        ...
        p_vis = np.random.binomial(1,0.5,(1,self.sizes['pen']+self.sizes['lbl']))

            
        for _ in range(self.n_gibbs_gener):
            p_vis[:, -self.sizes['lbl']:] = lbl
            p_h, h = self.rbm_stack['pen+lbl--top'].get_h_given_v(p_vis)
            p_vis, v = self.rbm_stack['pen+lbl--top'].get_v_given_h(p_h)

            p_v2, v2 = self.rbm_stack['hid--pen'].get_v_given_h_dir(v[:, :-self.sizes['lbl']])
...
#           vis = np.random.rand(n_sample,self.sizes["vis"])
        ...
        anim = stitch_video(fig,records).save("vids/%s.generate%d.mp4"%(name,np.argmax(true_lbl)))            
    ...
    def train_greedylayerwise(self, vis_trainset, lbl_trainset, n_iterations):
        ...
        try :
            ...
            p_h,h = self.rbm_stack['vis--hid'].get_h_given_v(vis_trainset)
            ...
            p_h1,h1 = self.rbm_stack['hid--pen'].get_h_given_v(h)
            ...
            h_label = np.concatenate((h1,lbl_trainset),axis=1)
            self.rbm_stack['pen+lbl--top'].cd1(h_label,n_iterations)
